...
let nodeGroup = null; // global variable fo nodes - circle & image
...
const init = function (graph) {
    ...
    const image = nodeGroup
        ...
        .attr("href", (d) => {
            if (d.data.id != "null" && d.data.id != "root") {
                return d.data.href;
            }
        })
    ...
    res_list.forEach(function (d) {
        d.addEventListener("click", function (e) {
            ...
            nodeGroup.selectAll("image").style("opacity", (d) => {
                if (d.data.name !== "root" && typeof d.data.id !== "undefined") {
            ...
            for (var i in arrlist) {
                if (d.data.id.startsWith(arrlist[i])) {
                        ...
                        for (var j in Object.values(d.data)) {
                            if (
                                ...
                                if (Object.keys(d.data)[j] === "id") {
                                    ...
                                    rtag.appendChild(newdivc);
                                } else if (Object.keys(d.data)[j] === "route_table") {
                                    ...
                                    row.innerHTML = "Dest_Cidr";
                                    header.appendChild(row);

                                    row = document.createElement("th");
                                    row.innerHTML = "Dest_Id";
                                    header.appendChild(row);
                                    ...
                                    for (var k in Object.values(d.data)[j]) {
                                        ...
                                        col.innerHTML = Object.values(d.data)[j][k].destination_cidr_block;
                                        row.appendChild(col);

                                        if (Object.values(d.data)[j][k].gateway_id !== undefined) {
                                            col = document.createElement("td");
                                            col.innerHTML = Object.values(d.data)[j][k].gateway_id;
                                            row.appendChild(col);
                                        } else if (Object.values(d.data)[j][k].nat_gateway_id !== undefined) {
                                            col = document.createElement("td");
                                            col.innerHTML = Object.values(d.data)[j][k].nat_gateway_id;
                                            row.appendChild(col);
                                        } else if (Object.values(d.data)[j][k].instance_id !== undefined) {
                                            col = document.createElement("td");
                                            col.innerHTML = Object.values(d.data)[j][k].instance_id;
                                            row.appendChild(col);
                                        } else if (Object.values(d.data)[j][k].network_interface_id !== undefined) {
                                            col = document.createElement("td");
                                            col.innerHTML = Object.values(d.data)[j][k].network_interface_id;
                                            row.appendChild(col);
                                        }

                                        col = document.createElement("td");
                                        col.innerHTML = Object.values(d.data)[j][k].state;
                                        row.appendChild(col);

                                        table.appendChild(row);
                                    }

                                    rattr.appendChild(table);
                                    ...
                                    if (typeof Object.values(d.data)[j] === "object") {
                                        ...
                                        newdivc.textContent =
                                            Object.keys(d.data)[j] + " : " + Object.values(d.data)[j];
                                    }
                                    rattr.appendChild(newdivc);
                                }
                            }
                    ...
                    newDIV.addEventListener("mouseover", function (event) {
                        const resource_id = event.target.innerHTML;

                        nodeGroup
                            .selectAll("circle")
                            .filter((d) => d.data.id === resource_id)
                            .style("stroke-width", "3px");
                    });
                    newDIV.addEventListener("mouseout", function (event) {
                        nodeGroup.selectAll("circle").style("stroke-width", "");
                    });
    ...
    function zoomTo(v) {
        ...
        circle
            .on("mouseover", (event, d) => {
                linkGroup
                    .selectAll("path")
                    .filter((l) => d.data.id === l.source || d.data.id === l.target)
                    .style("opacity", 0.6);
            })
            .on("mouseout", (event, d) => {
                linkGroup.selectAll("path").style("opacity", 0.2);
            });
    }
...
function render() {
    ...
    }).then(function (data) { console.log(data); console.log(init(data)); init(data)});
}
